...
public class ReceitaBean implements Serializable {
	...
	public String salvar() {
		...
		if (receita.getVal_Transação() <= 0) {
			...
			if (rDao.salvarReceita(receita)) {
				...
				conta.setVal_Conta(conta.getVal_Conta() + receita.getVal_Transação());
	...
	public String redirecionaReceita() {
		...
		valorAnt = receita.getVal_Transação();
		return "editaReceita";
	}
	...
	public String editarReceita() {
		...
		if (receita.getVal_Transação() <= 0) {// testa se o valor é menor ou iguala a zero
			...
			if (getCodContaAnt() == conta.getCod_Conta()) {// testa se a conta nao mudou
				...
				if (receita.getVal_Transação() == getValorAnt()) {// testa se o valor nao mudou
					rDao.atualizarReceita(receita);// testado - não mudou a conta e nem mudou o valor
					receita = new Receita();
					PrimeFaces.current().dialog().showMessageDynamic(message);

				} else {// neste caso o valor mudou
					conta.setVal_Conta(conta.getVal_Conta() - valorAnt + receita.getVal_Transação());
					cDao.atualizarConta(conta);
