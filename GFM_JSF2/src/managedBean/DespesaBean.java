...
public class DespesaBean implements Serializable {
	...
	public String salvarDespesa() {
		...
		if (despesa.getVal_Transação() <= 0) {
			...
			if (conta.getVal_Conta() < despesa.getVal_Transação()) {
			...
			conta.setVal_Conta(conta.getVal_Conta() - despesa.getVal_Transação());
	...
	public String redirecionaDespesa(Despesa despesa) {
		...
		valorAnt = despesa.getVal_Transação();
		return "editaDespesa";
	}
	...
	public String editarDespesa() {
		...
		if (despesa.getVal_Transação() <= 0) {// testa se o valor é menor ou igual a zero
			...
			if (getCodContaAnt() == conta.getCod_Conta()) {// testa se a conta nao mudou
				...
				if (despesa.getVal_Transação() == getValorAnt()) {// testa o valor permanece
					...
					conta.setVal_Conta(conta.getVal_Conta() + valorAnt - despesa.getVal_Transação());
					cDao.atualizarConta(conta);
